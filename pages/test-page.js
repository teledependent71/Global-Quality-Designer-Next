  ...
  return (
    <>
      <div className="test-page-container">
        ...
        <DataProvider
          renderSuccess={(context_ocqkbv) => (
            <>
              <h1 id={context_ocqkbv?.id}>Heading</h1>
            </>
          )}
          initialData={props.contextOcqkbvProp}
          persistDataDuringLoading={true}
          key={props?.contextOcqkbvProp?.id}
        />
      </div>
      <style jsx>
        {`
...
export async function getStaticProps(context) {
  try {
    const contextOcqkbvProp = await getEntityByAttribute({
      ...context?.params,
    ...
    return {
      props: {
        contextOcqkbvProp: contextOcqkbvProp?.data?.[0],
      },
      revalidate: 60,
    }
  } catch (error) {
